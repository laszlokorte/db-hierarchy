...
class HierarchyParamConverter implements ValueResolverInterface
{
    ...
    public function resolve(Request $request, ArgumentMetadata $configuration): \Generator
    {
        ...
        $attribute = $configuration
                    ->getAttributes(HierarchySlug::class, ArgumentMetadata::IS_INSTANCEOF)[0]
                    ?? null;

        $slug = $attribute?->slug;

        if (StorageConnection::class === $configuration->getType()) {
            $object = $this->schemaLoader->loadStorageConnection($request->attributes->get($slug ?? 'hierarchySlug'));
            ...
            $schemaSlug = $request->attributes->get($slug ?? 'hierarchySlug', 'system');
